...
| # | 항목 | 비고 |
|---|---|---|
| 1 | ~~`_gate_aggregate.py` 의 정규화 arm 처리 확인~~ → **확인 완료, §6.1 참조** | 역변환하지 않는다. 이번 판정에는 영향 없으나 하니스 재사용 시 함정 |
...
### 6.1 이 평가 하니스는 정규화 arm 을 역변환하지 않는다 (2026-09-08 확인)

`infer_phase1_pbc.py` 가 예측·정답에 거는 유일한 변환은 `normalize_channels_to_bx_by_a_je`
(`phase1_static/contracts.py:273`)인데, 그 함수는 이름과 달리 **통계 정규화가 아니라 채널 레이아웃
매핑**이다 — 3채널 `[A,Bx,By]` 를 `[Bx,By,A,Je=0]` 로 재배열하는 식이고, 예측과 정답에 동일하게
적용된다. **z-score 역변환은 어디에도 없다.**

그래서 정규화 arm 의 `rel` 은 두 원인을 구분하지 못한다 — 모델이 스킬이 없어도 1.0 이고,
모델이 정규화 공간의 값을 내놓는데 원시 정답과 비교해도 1.0 이다. 이번 판정이 학습 손실
(§4 두 번째 줄)에 기대는 이유가 이것이다.

**이 하니스를 정규화 arm 에 재사용하지 말 것.** 쓰려면 체크포인트에 저장된 통계로 출력을
역변환하는 단계를 `infer_phase1_pbc.py` 에 먼저 넣어야 한다.
