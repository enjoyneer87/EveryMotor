...
전 스텝 cold start 후 Arkkio 토크를 export 장의 토크와 대조:

    평균 -0.180%   RMS 0.283%   최대 |0.697%|   (45/45 수렴)
...
초기값만 바꿔 비선형 반복수와 wall time을 비교했다(case_0004, 45 자세):

    초기값                       평균 반복  중앙값  총 반복  평균 시간
    zero (a0 = 0)                  17.44     17      785    1.160 s
    직전 회전각 (수렴해 이월)       9.60      9      432    0.538 s
    AI (curl-A ckpt, §19)          15.22     16      685    0.856 s

**판정: 음성.** curl-A 초기값은 zero 대비 반복 -12.7%(시간 -26%)로 도움은 되지만,
**실무 표준인 직전각도 warm-start(9.60회)에 크게 못 미친다**(15.22회, +59%).
