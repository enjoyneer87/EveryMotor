"""Patched datasets.py: adds CachedMotorDataset and CachedMotorDataModule for
loading pre-processed .pt files instead of recomputing Delaunay each epoch."""
...
class CachedMotorDataset(Dataset):
    """Map-style dataset that loads pre-processed .pt files from a cache directory.
    Much faster than MSOIterableDataset since Delaunay triangulation is pre-computed.
    Also supports num_workers > 0 in DataLoader without data duplication issues.
    """
    def __init__(self, cache_dir: str | Path, split: str, transform=None) -> None:
        ...
        assert len(self.files) > 0, f"No .pt files in {self.cache_dir}"
...
class CachedMotorDataModule(pl.LightningDataModule):
    """DataModule using pre-cached .pt files for fast data loading."""
    def __init__(self, cfg: DictConfig | dict, cache_dir: str | Path) -> None:
        super().__init__()
        self.cfg = cfg
        self.train_dataset = CachedMotorDataset(cache_dir, split="train", transform=cfg.data_transform)
        self.val_dataset = CachedMotorDataset(cache_dir, split="val", transform=cfg.data_transform)
        self.test_dataset = CachedMotorDataset(cache_dir, split="test", transform=cfg.data_transform)
