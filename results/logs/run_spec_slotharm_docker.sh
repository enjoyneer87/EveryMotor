#!/bin/bash
# SPEC: slot-harmonic band spectral basis, pre-registered in methodology_review section 36.
#
# SINGLE-AXIS DISCIPLINE: identical to run_spec_slotharm_docker.sh (the R9 champion recipe) except ONE
# added training flag, --extra-spectral-orders 48,96,144. Same data, same split, same model, same
# weights, same epochs. Baseline for the comparison is R9 ep50 (legacy6 pooled:
# |B| 11.302 / airgap 6.925 / torque 3.799).
#
# Why these three orders (section 31d): the admissible basis is odd multiples of 4, so the 48-slot
# machine's slot harmonics 48/96/144 are NOT in it -- yet an a1-only basis study measured them to be
# physically present: 48 params 9.034% -> 54 params (adding exactly these) 8.291%. Six parameters
# recover 0.74pp, 36% of the total 2.08pp achievable by going to a 400-parameter free basis.
# The spectral loss therefore cannot see a structure the field really has.
#
# Pre-registered decision rule (section 36), LEGACY 6-test group, pooled, vs R9 ep50; band +-0.25pp:
#   airgap < 6.675%   -> SUCCESS: slot-harmonic supervision is a live lever; adopt and re-baseline
...
#
# KNOWN CONFOUND, recorded before running: the flag does two things at once -- it adds supervision
# targets AND redistributes the fixed --band-spectral-weight 30 across 27 orders instead of 24
# (~11% less weight per coefficient). A null result is therefore not clean evidence against slot
# harmonics; the follow-up would re-run at a weight scaled by 27/24.
#
...
BAND_W="${BAND_W:-30}"
EPOCHS="${EPOCHS:-50}"
TAG="${TAG:-doe_v3_bw30_prior_slotharm}"
...
echo "=== SPEC pre-flight 1: split digest + sizes ==="
python - <<'EOF'
import json, sys
...
EOF
[ $? -eq 0 ] || { echo "SPEC PREFLIGHT FAILED -- split contract"; exit 1; }

echo "=== SPEC pre-flight 2: prior cache complete for every case ==="
python - <<'EOF'
import json, sys
...
EOF
[ $? -eq 0 ] || { echo "SPEC PREFLIGHT FAILED -- prior cache"; exit 1; }
echo "PREFLIGHT OK"

echo "=== SPEC train (extra orders 48,96,144)  480 cases  BAND_W=${BAND_W}  epochs=${EPOCHS} ==="
python -u train_doe_curl_mgn.py \
  ...
  --extra-spectral-orders 48,96,144 \
...
if [ "${TRAIN_EXIT}" -ne 0 ]; then
  echo "SPEC TRAIN FAILED -- skipping scoring. Resume with:"
  echo "  RESUME=${CKPT}.last bash results/logs/run_spec_slotharm_docker.sh"
  exit 1
fi

echo "=== SPEC scoring: all 18 test cases, then each group with its own v3-digest split ==="
for pair in "${SPLIT}:${SCORE}" \
  ...
  [ "${rc}" -eq 0 ] || { echo "SPEC SCORING FAILED on ${out}"; exit 1; }
done

echo "SPEC SLOTHARM DONE"
